...
async def process_webhook_event(event: dict, user_id: str, reply_token: str, line_access_token: str, api_key: str, logger: JodNidLogger):
    ...
    if event_type == "message":
        ...
        if msg_type == "text":
            ...
            logger.info(module="webhook", message=f"handle text message: {user_text}", user_id=user_id)
            ...
            await handle_text_message(user_id, user_text, reply_token, line_access_token, api_key, logger)

        # --- กรณีเป็น Image (งานที่หนักที่สุด) ---
        elif msg_type == "image":
            message_id = message.get("id")
            logger.info(module="webhook", message=f"handle image message id: {message_id}", user_id=user_id)
            # is_ocr_active = get_config_value(key="is_ocr_active")
            # if is_ocr_active:
            #     send_push_notification(user_id, content="ระบบกำลังปรับปรุง กรุณาลองใหม่อีกครั้งในภายหลัง")
            #     return
            # ย้าย Logic การทำ OCR และ Extract ไปไว้ในฟังก์ชันย่อย
            await handle_image_message(user_id, message_id, reply_token, line_access_token, api_key, logger)

    elif event_type == 'postback':
        postback_data = event.get("postback", {}).get("data")
        logger.info(module="webhook", message=f"handle postback data: {postback_data}", user_id=user_id)
        # Logic การจัดการ Confirm/Cancel
        await handle_postback(postback_data, user_id, logger)


async def handle_text_message(user_id: str, user_text: str, reply_token: str, line_access_token: str, api_key: str, logger: JodNidLogger):
    try:
        ...
        logger.info(module="webhook_text_ai", message=f"processing text message: {user_text_lower}", user_id=user_id)

        # --- 1. คำสั่งช่วยเหลือ (Help/Guidance) ---
        if any(k in user_text_lower for k in HELPER_KEYWORDS):
            logger.info(module="webhook_text_ai", message=f"processing helper keywords", user_id=user_id)
        ...
        # --- 2. คำทักทาย (Greetings) ---
            ...
            logger.info(module="webhook_text_ai", message=f"processing greeting keywords", user_id=user_id)
            profile = get_line_profile(user_id=user_id, line_token=line_access_token)
        ...
        if is_simple_format:
            ...
            logger.info(module="webhook_text_ai", message=f"processing has number text message", user_id=user_id)
        ...
        if is_transaction:
            logger.info(module="webhook_text_ai", message=f"processing transaction message", user_id=user_id)
            ...
            logger.info(module="webhook_text_ai", message=f"Not a transaction message.", user_id=user_id)
            guidance_text = (
        ...
        logger.error(module="webhook_text_ai", message=f"Error handling text message: {str(e)}", user_id=user_id)
        send_push_notification(user_id, content="เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง")

async def handle_image_message(user_id: str, message_id: str, reply_token: str, line_access_token: str, api_key: str, logger: JodNidLogger):
    ...
    if processing_image is None:
        logger.error(module="webhook_image_ai", message=f"Failed to download image", user_id=user_id)
    ...
    if save_file:
        ...
        if not status:
            logger.error(module="webhook_image_ai", message=f"OCR failed: {ocr_json.get('error')}", user_id=user_id)
        ...
        logger.error(module="webhook_image_ai", message=f"Failed to save image", user_id=user_id)
        send_push_notification(user_id, content="ขออภัยครับ ระบบไม่สามารถอ่านข้อมูลจากรูปนี้ได้ กรุณาลองใหม่อีกครั้งด้วยรูปที่ชัดเจนขึ้นครับ")
...
async def handle_postback(postback_data: str, user_id: str, logger: JodNidLogger):
    try:
        ...
        logger.info(module="webhook_postback", message=f"processing postback action: {action}, temp_id: {post_temp_id}", user_id=user_id)
        ...
        if action == "confirm":
            ...
            if result:
                ...
                logger.info(module="webhook_postback", message=f"text_confirm: {text_confirm}", user_id=user_id)
                ...
                if budgets:
                    for b in budgets:
                        ...
                        logger.info(module="webhook_postback", message=f"budget_msg: {budget_msg}", user_id=user_id)
                ...
                logger.info(module="webhook_postback", message=f"❌ ไม่พบข้อมูลรายการนี้ หรือถูกบันทึกไปแล้วครับ temp_id: {post_temp_id}", user_id=user_id)
...
def confirme_data_from_edit(post_temp_id: str, user_id: str, items: List[Dict[str, Any]] = None, logger: JodNidLogger = None):
    if logger is None:
        logger = JodNidLogger()
    ...
    try:    
        ...
        if result:
            ...
            text_confirm = f"✅ บันทึกสำเร็จ {count} รายการ\n💰 ยอดรวม ฿{total:,.2f}"
            send_push_notification(user_id=user_id, content=text_confirm, alt_text="บันทึกสำเร็จ")
            logger.info(module="webhook_postback_edit", message=f"text_confirm: {text_confirm}", user_id=user_id)
            # 3. ส่ง Push Message สรุปงบประมาณ (ถ้ามีการตั้งงบไว้)
            if budgets:
                for b in budgets:
                    # คำนวณสถานะ
                    ...
                    logger.info(module="webhook_postback_edit", message=f"budget_msg: {budget_msg}", user_id=user_id)
                    # ส่งเป็น Push Message (เพราะอาจจะใช้เวลาประมวลผลแยกกัน)
                    send_push_notification(user_id, content=budget_msg, alt_text="สรุปยอดใช้จ่าย")
            ...
            logger.info(module="webhook_postback_edit", message=f"❌ ไม่พบข้อมูลรายการนี้ หรือถูกบันทึกไปแล้วครับ temp_id: {post_temp_id}", user_id=user_id)
            send_push_notification(user_id=user_id, content="❌ ไม่พบข้อมูลรายการนี้ หรือถูกบันทึกไปแล้วครับ", alt_text="ไม่พบข้อมูล")
    except Exception as e:
        logger.error(module="webhook_postback_edit", message=f"Error handling postback: {str(e)}", user_id=user_id)
        send_push_notification(user_id, content="ขออภัยครับ เกิดข้อผิดพลาดในการประมวลผล")
